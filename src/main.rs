...
fn main() {
    ...
    let text_numbers = Text::new(&(1..=20).map(|n| format!(" {} \n", n)).collect::<String>())
        .max_width(4)
        .with_comment("numbers");

    let window = Container::new(
        Rect::new(Size::new(width, height - 2)),
        Stack::Horizontally,
    )
    .with_comment("window with text file and numbers")
    .stack(Stack::Horizontally)
    .push(text_numbers)
    .push(text_field);
    ...
    let revi = Container
        ::new(Rect::new(Size::new(width, height)), Stack::Vertically)
