...
impl ReVi {
    ...
    pub fn execute(&mut self, count: usize, commands: &[ReViCommand]) -> (InputState, Vec<ui::Render>) {
        ...
        for command in commands {
            match command {
                ReViCommand::StartUp => {
                    ...
                    render_commands.push(ui::Render::Window{pos: window.window_offset, text: window.to_string()});
                    render_commands.push(ui::Render::StatusBar{pos: window.status_bar_pos(), text: window.status_bar()});
                ...
                ReViCommand::CursorUp => {
                    if self.focused_window_mut().move_cursor_up(count) {
                        render_commands.push(ui::Render::Window{pos: self.focused_window().window_offset, text: self.focused_window().to_string()});
                    }
                    render_commands.push(ui::Render::StatusBar{pos: self.focused_window().status_bar_pos(), text: self.focused_window().status_bar()});
